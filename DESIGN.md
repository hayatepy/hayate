...
- **文字列インターニング**: ヘッダ名の `sys.intern` で dict 照合をポインタ比較化 — 遅延デコードで大半のヘッダはそもそも触れないため、計測で必要が出るまで**見送り**
- **オブジェクトプール**: Context/Request の再利用 — CPython の slotted オブジェクト割り当ては十分速く、リーク・状態残留リスクが利得を上回るため**不採用**
...
- **プラットフォーム応答のパススルー(escape hatch)**: DO スタブ応答の `webSocket` プロパティのような**プラットフォーム拡張は hayate Response を通すと失われる**。Rack 型の「素通し」を許す設計案は、ミドルウェアの応答加工と衝突する(素通し応答にはヘッダ staging が効かない)ため、**証拠(実ユーザーの要望)が出るまで見送り**。現行の回避策: WS は外側 worker で終端するか、DO 直行ルーティングにする(公式 chatroom example と同型)

**結論**: ゼロインピーダンスは言語境界がある限り不可能。hayate の答えは (1) 遅延ラッパー(触れないものは変換しない)、(2) 信頼済み fast path(検証済みのものは再検証しない)、(3) 交差バッチング(渡るときは一度で渡る)、(4) SIMD 加速(どうしても走査するなら最速で)の 4 原則で、いずれも実測を添えて採用した。
