...
- コアは **runtime 依存 1 個**(`uts46`、pure Python、推移依存なし)、
  async-first、PEP 695 世代の型付け。標準準拠を直接担う小さな依存だけを許す。
...
Fetch 標準の**意味論**(不変条件・状態遷移・エラー条件)に準拠した自前実装。
URL の Unicode host table だけは focused dependency に委譲する。
...
- `hayate-msgspec` / `hayate-pydantic` を別配布(重量級かつ選択的な依存をコアから分離する)。
...
コアが I/O を持たない純粋関数であることが、アダプタを「変換だけの薄い層」に保つ鍵。逆に言えば、コアに I/O や ASGI 概念が漏れた瞬間にこの表は崩れる。
...
| 2: native accelerator | Rust(maturin)による **opt-in** 拡張(`accel/` = `hayate-accel`)。初弾は compact JSON encoder(dynamic-json を 0.99x → 1.22x に改善)。第二弾は multipart 境界走査(`memchr::memmem` の SIMD 探索 + ペイロード単一コピー、accel 0.2.0)— **10.5MB ボディで 11x**(5.7ms → 0.5ms)。意味解析は Python 側の単一経路に残し、分割器のみ交換(パリティテストで固定)。次候補は計測で選定(SFV、URL/IDNA) | native CPython(abi3 wheel、pyo3 0.29)。**Workers は PyEmscripten wheel をサポートするため、emscripten ターゲットもビルドできれば Pyodide でも有効**(未検証・将来項目) |
...
### 14.4 境界インピーダンス — 各フレームワークの先行事例と hayate の決定(2026-07-22)

**問題**: hayate の Request/Response はプラットフォームのオブジェクトと別物なので、境界(ASGI、Workers FFI)で常に変換を払う。Hono は「フレームワークの Response = プラットフォームの Response」(JS on JS)でこの問題自体が存在しない。Python である以上ゼロにはできない — できるのは変換の**回数と厚みの最小化**。

**先行事例の調査**(ソースで確認):

| フレームワーク | 解法 | hayate への含意 |
|---|---|---|
| Starlette | Request は ASGI scope の**遅延ラッパー**。Response は `raw_headers` を wire 形式で保持し、**Response 自身が ASGI callable**(自分でトランスポートに書き出す) | 遅延ラッパーは Tier 1 で採用済み。「応答が自分を書き出す」は変換層の消去と等価 |
| Django | HttpRequest は WSGI environ の遅延ラッパー(`META` は素の environ)。独自オブジェクト + 境界で一度だけ薄く変換 | 同上(WSGI 世代の同型解) |
| Rack / Rails | 応答は具象クラスでなく `[status, headers, body]` **プロトコル** | 「何であれ応答になれる」= 変換の trait 化 |
| Axum(Rust) | `IntoResponse` / `FromRequest` trait。変換を型ごとに実装しコンパイル時に単相化 | Python では import 時の実装選択(jsonutil / formdata 方式)が相当 |
| 公式 workers-py ASGI ブリッジ | ヘッダは `Object.fromEntries(headers)` で **1 交差**。ただしリクエストボディはアプリ実行前に**全量バッファ**、応答ストリームは TransformStream + writer(push 型) | 1 交差ヘッダは正しいので採用。ボディは hayate(遅延 pull 型)が優位 — 追随しない |

**低レイヤー選択肢の棚卸し**(採否):

- **交差のバッチング(採用、0.4.1)**: FFI 交差は 1 回あたり数百 ns〜µs(proxy トランポリン)。応答ヘッダ N 回の `append()` を、`js.Headers.new(pairs)` に配列を渡す 1 交差に置換。SDK 自身の `_to_js_headers` も同じ受け口を持つ
- **信頼済み fast path(採用、0.4.1)**: プラットフォームが検証済みの入力を再検証しない。URL は成分分割のみ(2.49µs → 0.25µs、10x)、ヘッダは `_from_trusted_pairs`(JS Headers の列挙は小文字保証)、ボディは Fetch の null body 規約でスキップ(bodyless GET の非同期 FFI 往復を丸ごと消去)
- **SIMD バイト走査(採用、accel 0.2.0)**: multipart 境界探索を `memchr::memmem` へ。CPython の `bytes.split`(Crochemore-Perrin、非 SIMD)対比 11x
- **文字列インターニング**: ヘッダ名の `sys.intern` + 辞書キャッシュで `lower()` を回避する案 — **実測で棄却が確定**(0.5.0 検証)。`str.lower()` は既小文字入力でも新規確保するが 32ns で、dict get + フォールバックの 40ns に勝つ。CPython の短い ASCII lower はほぼ memcpy であり、ハッシュ+プローブより速い。再訪不要
- **wire エンコードキャッシュ(採用、0.5.0)**: 応答ヘッダ対の latin-1 エンコードは毎リクエスト同一対を再計算していた — 有界 memo 化で 114ns → 41ns/対。同系で `Headers.get()` の単値 fast path(181→145ns)、ルータ static ヒットの共有空 params(40→31ns)。合算はマクロベンチ +1〜2%(static-text 239k req/s)
- **メソッド別 dynamic 索引**: 64 ルート最悪ケースで 4.92→4.61µs(6%)— regex 実行が支配的で構造変更に見合わず**棄却**(数値で恒久記録)
- **ルータのスケーリング(0.5.0 後に特性評価 → 0.6.0 で採用)**: (a) **平坦 alternation(全ルートを 1 本の `|` 結合)は棄却が確定** — 1024 ルート最終マッチで線形 76µs vs 結合 672µs と**9 倍悪化**。個別コンパイルは sre のリテラル接頭辞スキップで高速棄却できるが、alternation 内の分岐はそれを受けられない(Hono RegExpRouter が JS で勝つ本質は接頭辞の trie 因数分解であって結合ではない)。(b) **セグメント trie を 0.6.0 で採用**: static dict → trie(プレーン `:name` ルート)→ regex 尾部(制約付き・optional・wildcard)の 3 段。**登録順優先は登録インデックスの比較で厳密に保存**(tier 分けはアプリから不可視 — tests/test_router.py で固定)。greedy 走査 + 分岐点でのみスタック実体化で、**ルート数に依存せず 0.95µs フラット**(1024 ルートで旧線形の 80 倍、many-routes(64) ベンチ 102k → 200k req/s、vs Starlette 1.93x → **3.83x**)。トレードオフも記録: 動的ルートが 1 本だけのとき旧単発 regex の 0.5µs に対し 0.94µs(dynamic-json ベンチ -8%)— 線形 1 本目ヒットは regex の絶対最速ケースであり、2 ルート以上の実アプリでは trie が広く勝つため、適応昇格機構(閾値未満は線形)は複雑さに見合わず**不採用**
- **終端パラメータ索引(0.11、採用)**: `/literal/:name` のうち、先行する同一 method の plain/regex 動的ルートと重ならないことを登録時に証明できる形だけを prefix lookup へ昇格。単一 dynamic の trie 走査を約 0.50µs → 0.25µs に短縮する。先行 general route、先行 regex、同形 route は従来の登録順を厳密に維持し、証明できない形は既存 trie/regex 経路へ落とす
- **Workers の最終状態コンストラクタ(0.11、採用)**: platform がすでに正規化した request/context/text response は、公開 constructor を実行後に内部状態を上書きせず、private constructor で最終 lazy state を一度だけ代入する。実 workerd/Pyodide 内の計測で adapter core は約 1.9–2.0µs、完全な Hayate text path は約 11.6–12.4µs。一方 HTTP 全体は約 570µs/request。正式 20 connections × 10秒 × 3 round では global が raw Python ceiling の **99.94% throughput**、CPU/request +2.05%、class が direct-JS class control の 98.74% に到達した。以後は意味論を削る micro optimization ではなく runtime/FFI 境界として追跡する
- **UTS-46 の遅延 import(0.11、採用)**: 約 136KiB の mapping table は non-ASCII special host の構築時だけ import する。ASCII URL と platform-validated request URL の startup ではロードしない。パッケージ自体は bundle に残し、306/306 の WHATWG URL 準拠を維持する（除外して benchmark 数字だけを縮める案は棄却）
- **オブジェクトプール**: Context/Request の再利用 — CPython の slotted オブジェクト割り当ては十分速く(~50ns)、`wait_until` やストリームがリクエスト後も参照を握れる以上エスケープ解析が不可能で、リーク・状態残留リスクが利得を上回るため**不採用**(恒久)
- **Response 自己書き出し(Starlette 型)**: アダプタが 3 つ(ASGI/Workers/Lambda)ある現状では、Response にランタイム知識を持たせるより**アダプタ側に変換を置く現行構造が正しい**(コアの純関数性を守る)。不採用
- **プラットフォーム応答のパススルー(採用、0.5.0)**: `hayate.adapters.workers.forward(c, fetcher)` — 原プラットフォームリクエストを Fetcher(DO スタブ / Service Binding)へ素通し転送し、応答を**無変換で**返す。`webSocket` などのプラットフォーム拡張が保存されるため、**外側 hayate アプリの通常ルートを貫通して DO 内の `@app.ws()` ハンドラに到達する WebSocket が成立**(実機検証済み: ローカル workerd + 本番)。制約は明示: 素通し応答にはレスポンスミドルウェアの staging(`c.header()`)が効かない(docstring と docs に記載)。コアは無変更 — アダプタ内の `Response` サブクラスが印を付け、境界で原応答を返すだけ

**結論**: ゼロインピーダンスは言語境界がある限り不可能。hayate の答えは (1) 遅延ラッパー(触れないものは変換しない)、(2) 信頼済み fast path(検証済みのものは再検証しない)、(3) 交差バッチング(渡るときは一度で渡る)、(4) SIMD 加速(どうしても走査するなら最速で)、(5) **素通し(変換できない拡張は変換しない)**の 5 原則で、いずれも実測を添えて採否を決めた。棄却した案も数値付きで残す — 再訪コストをゼロにするため。

**transport 境界の再計測(2026-07-26)**: 競合 benchmark の4 workloadを
Hayate と最小 raw ASGI の両方で、同じ lock 済み Uvicorn 0.51.0 /
asyncio / h11 上に載せて 50 connection × 10秒 × 3 round で測定した。
raw ASGI 16,094 req/s に対し Hayate は 13,438 req/s、**raw ceiling の
83.5%**(workload 別 81.2–87.1%)。同じ run の Hono は 63,069 req/s で、
Hayate 比 4.69x だが raw Uvicorn 比でも 3.92x だった。したがって現時点の
Hono 差の大半は core のルータ/Context ではなく runtime/transport 境界にある。
Fetch 意味論を崩して残り16.5%を追うより、framework 税と transport 税を別系列で
継続計測し、ASGI server の選択や Workers 直結を利用者が比較できる形にする。

**条件付き項目の証拠収集(2026-07-22 実施)**: 「外部の証拠を待つ」項目は、(a) 合成特性評価で今測れるもの、(b) 計測点を仕込んで将来の需要を数えられるようにするもの、に分解して先回りした。

- ルータ: 上記のとおり合成ベンチで決着(平坦 alternation 棄却 / セグメント trie 採用候補)。外部証拠は不要になった
- IDNA: 2026-07-22 の単純な非 ASCII 配線は +2 ケースだけだったが、2026-07-24 に
  WHATWG 専用 `uts46.whatwg.domain_to_ascii` を percent-decode → strict UTF-8 の正しい順で
  配線すると **246→306/306(80.4%→100%)** を実測。`uts46` 0.2 は約 190 KiB の
  pure-Python wheel、推移依存なしで、Pyodideにもそのまま bundle できる。
  **決定を更新**: 技術的完全性を優先し v0.10 で唯一の runtime 依存として採用。
  ASCII の invalid A-label を保持する WHATWG 挙動と forbidden percent もWPTで固定。
...
| **v0.3** | validator フック(実装済み — callable プロトコルにより msgspec / pydantic が**アダプタパッケージなしで直結**)/ Workers アダプタ / Lambda アダプタ / ドキュメントサイト(https://hayatepy.dev/ — MkDocs Material + Pyodide プレイグラウンド + llms.txt)— **すべて完了** | 同一アプリが uvicorn と Workers で無変更動作 ✅(2026-07-22、ローカル workerd で実証。実機で発見した workers-py ラッパー形状の差異は 0.3.1 で修正) |
...
| **v0.10** | WHATWG special-host の percent-decode → strict UTF-8 → non-transitional UTS-46 と完全 path encode set — **完了(2026-07-24)** | URL の対象 WPT **306/306(100%)**。`uts46` は pure Python / 推移依存なし、CPython・Pyodide/workerd の両方で検証 |
| **v0.11** | native Workers の global compatibility path、最終 lazy state 構築、終端パラメータ索引、UTS-46 遅延 import、競合 benchmark、エコシステム HEAD compatibility gate — **完了(2026-07-26)** | global path は raw Python throughput ceiling の **99.94%**、class path は direct-JS class control の **98.74%**。0.11 RC wheel を auth/fetch/MCP/OpenAPI/create-hayate HEAD に注入し、CPython strict contract と実 workerd の MCP/生成 Workers を全て通過 ✅ |
...
1. **準拠ラチェット維持**: wpt スイートで URLPattern supported subset の behavior mismatch 0 / URL **306/306** を維持(引き下げ不可)。docs/conformance.md がリリースごとに最新
...
6. **凍結直前の API 監査の再実施**: 下記「公開 API 監査」を宣言直前にもう一度機械実行し、追加分も基準を満たすことを確認する

**公開 API 監査(2026-07-22 実施、0.4.1)**: 凍結対象 = `hayate.__all__` 27 名 + `hayate.middleware` 10 名 + アダプタ 4 名(`to_workers` / `to_durable_object` / `to_lambda` / `ASGIAdapter`)。機械監査(export 全数 × docstring 有無 × `__all__` 外の公開名リーク)で確認し、欠落 docstring 18 件を補填、`SSEMessage` 型を export に追加。サブモジュール名が `dir(hayate)` に見えるのは Python の import 慣習どおりで対応不要と判断。`to_durable_object` の「factory 名 = クラス名」規約は明示 `name=` 引数を将来**加算的に**足せるため凍結の障害にならない。
