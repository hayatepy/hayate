...
- **Response 自己書き出し(Starlette 型)**: アダプタが 3 つ(ASGI/Workers/Lambda)ある現状では、Response にランタイム知識を持たせるより**アダプタ側に変換を置く現行構造が正しい**(コアのゼロ依存・純関数性を守る)。不採用
...
- IDNA: `idna` パッケージ(UTS-46)を試験配線して wpt 増分を実測 — **非 ASCII ホストへの単純適用は +2 ケースのみ**(202→204/306)。残り 102 失敗の内訳: **host の percent-decode → domain_to_ascii パイプラインが 57** / 厳格な拒否 26 / IPv6 正規化 9 / IPv4 正規化 5 / path 正規化系 5。テーブルコストは ~310 KiB(pure Python、Workers バンドル/スナップショットに直撃)。**決定**: (i) テーブル不要の純アルゴリズム約 40 ケース(IPv4/IPv6/数値ホスト拒否/path 正規化)を次の準拠作業とする(ゼロ依存のままラチェットを ~80% 台へ)、(ii) IDNA 本体は需要ゲート維持 — 拒否エラーメッセージを計測点化し、実ユースケースを [hayatepy/hayate#2](https://github.com/hayatepy/hayate/issues/2) に集約する(静かな離脱を数えられる報告に変換する仕組み)
