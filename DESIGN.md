...
- **ルータのスケーリング(0.5.0 後に特性評価済み — 待たずに測った)**: (a) **平坦 alternation(全ルートを 1 本の `|` 結合)は棄却が確定** — 1024 ルート最終マッチで線形 76µs vs 結合 672µs と**9 倍悪化**。個別コンパイルは sre のリテラル接頭辞スキップで高速棄却できるが、alternation 内の分岐はそれを受けられない(Hono RegExpRouter が JS で勝つ本質は接頭辞の trie 因数分解であって結合ではない)。(b) **セグメント trie(dict 走査)が採用候補として確定**: ルート数に依存せず **0.66µs で平坦**(1024 ルートで線形の 115 倍、8 ルートでも 1.04→0.66µs)。採用条件: 制約付き param(`:id(\d+)`)・optional・wildcard は regex 尾部に残す 3 段構成(static dict → trie → regex 線形)として実装し、登録順優先の意味論をテストで固定できること。many-routes(64) ベンチでも 5µs → 0.7µs 相当の実測改善が見込めるため、次のコア作業として採用可
