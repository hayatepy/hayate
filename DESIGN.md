...
- **終端パラメータ索引(0.11 候補、採用)**: `/literal/:name` のうち、先行する同一 method の plain/regex 動的ルートと重ならないことを登録時に証明できる形だけを prefix lookup へ昇格。単一 dynamic の trie 走査を約 0.50µs → 0.25µs に短縮する。先行 general route、先行 regex、同形 route は従来の登録順を厳密に維持し、証明できない形は既存 trie/regex 経路へ落とす
- **Workers の最終状態コンストラクタ(0.11 候補、採用)**: platform がすでに正規化した request/context/text response は、公開 constructor を実行後に内部状態を上書きせず、private constructor で最終 lazy state を一度だけ代入する。実 workerd/Pyodide 内の計測で adapter core は約 1.9–2.0µs、完全な Hayate text path は約 11.6–12.4µs。一方 HTTP 全体は約 570µs/request。正式 20 connections × 10秒 × 3 round では global が raw Python ceiling の **99.94% throughput**、CPU/request +2.05%、class が direct-JS class control の 98.74% に到達した。以後は意味論を削る micro optimization ではなく runtime/FFI 境界として追跡する
- **UTS-46 の遅延 import(0.11 候補、採用)**: 約 136KiB の mapping table は non-ASCII special host の構築時だけ import する。ASCII URL と platform-validated request URL の startup ではロードしない。パッケージ自体は bundle に残し、306/306 の WHATWG URL 準拠を維持する（除外して benchmark 数字だけを縮める案は棄却）
