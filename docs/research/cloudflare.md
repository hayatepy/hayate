...
引き続き class entrypoint とする。生データと全 sample は
[`benchmarks/competitive/workers/results/2026-07-26-macos-arm64.json`](../../benchmarks/competitive/workers/results/2026-07-26-macos-arm64.json)
に保存した。
...
- [x] **FFI の持続負荷と benchmark 分離** — **修正・検証済み(2026-07-26)**。同一 workload の Workers 競合 benchmark により、GET/HEAD の null body、request stream reader、abort listener、response headers の proxy lifetime を監査。GET/HEAD は body property 自体を読まず、reader lock と一時 `JsProxy` は決定的に破棄し、abort listener は application が signal を観測した場合のみ接続するよう修正した。Hayate の各公開 sample（20 connections・10 秒）は error/non-2xx 0 で完走。なお 30 秒連続 JSON POST は SDK-only raw Python Worker の公式 `request.json()` でも同じ Pyodide fatal error を再現したため、現行ローカル workerd/Pyodide 境界の累積問題として切り分け、throughput sample は target × scenario × round ごとに fresh Worker process を使う。Linux workerd では body 付き unmatched POST を未消費のまま 405 にすると次の Python Worker request が 500 になることも raw SDK control で再現したため、共通 contract の 405 case は method/Allow だけを検査し、未読 stream 処理を混在させない。runner は error、timeout、non-2xx を含む sample を拒否し、壊れた高 RPS 値を report へ採用しない
