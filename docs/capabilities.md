...
**Hayate advantaged** — Hayate has the clearest first-party path when one Python application must run on ASGI, native Cloudflare Workers, and AWS Lambda HTTP payload v2 while combining typed HTTP contracts, cached dependency graphs, MCP 2025-11-25, authorization, and checked SQLite/D1 access.
...
| **Native AWS Lambda HTTP payload path**<br>The framework package translates API Gateway HTTP API v2 and Function URL events directly, without a separately governed ASGI-to-Lambda adapter. | **Core** — to_lambda maps payload-v2 requests, responses, binary bodies, cookies, and background work directly to the Fetch core; CI installs the wheel in AWS's Python image and invokes it through the Runtime Interface Emulator. | **No first-party path** — FastAPI's deployment documentation delegates other cloud providers to provider guides; the framework package does not include an AWS Lambda event adapter. | **No first-party path** — Django documents WSGI and ASGI deployment and ASGI servers, not a first-party API Gateway or Function URL event adapter. | **Core** — hono/aws-lambda supplies the maintained Lambda handler, including binary response handling. |
...
### Native AWS Lambda HTTP payload path

- **Hayate — Core:** to_lambda maps payload-v2 requests, responses, binary bodies, cookies, and background work directly to the Fetch core; CI installs the wheel in AWS's Python image and invokes it through the Runtime Interface Emulator. ([evidence 1](https://github.com/hayatepy/hayate/blob/main/src/hayate/adapters/aws.py), [evidence 2](https://github.com/hayatepy/hayate/blob/main/tests/test_lambda_adapter.py), [evidence 3](https://github.com/hayatepy/hayate/blob/main/scripts/check_lambda_runtime.sh), [evidence 4](https://github.com/hayatepy/hayate/blob/main/examples/lambda/Dockerfile))
...
- **Hono — Core:** hono/aws-lambda supplies the maintained Lambda handler, including binary response handling. ([source 1](https://hono.dev/docs/getting-started/aws-lambda))
- **Relative to Hayate:** FastAPI: Hayate advantage; Django: Hayate advantage; Hono: Parity.
