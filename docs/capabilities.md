...
| **Typed request and response OpenAPI contracts**<br>Types drive input validation, response validation/serialization, OpenAPI 3.1, interactive docs, and client generation. | **First-party** — hayate-openapi 0.8.1 provides explicit source markers, portable typed constraints, typed binary files, response contracts, pluggable schema providers, OpenAPI 3.1.1, Scalar, and a generated dependency-free Fetch client on CPython and Python Workers. | **Core** — Type-driven validation, OpenAPI, JSON Schema, and interactive docs are central FastAPI features. | **External** — API schemas and typed REST contracts are provided by projects such as Django REST Framework rather than Django core. | **First-party** — @hono/zod-openapi combines validation, types, and OpenAPI generation. |
...
| **Maintained typed client path**<br>Server contracts produce a supported TypeScript client or types without manually duplicating request/response shapes. | **First-party** — hayate-openapi 0.8.1 generates a deterministic status-discriminated Fetch client with zero runtime package dependencies; create-hayate 0.13.0 drift-checks it, and golden-app strictly compiles and executes authenticated JSON, query/path, multipart, and delete calls against real ASGI. This is schema generation rather than Hono's in-process route-type inference. | **Core** — OpenAPI is built in and the documentation describes automatic client generation. | **External** — Typed API client generation depends on an external schema stack. | **Core** — Hono RPC infers input and output types directly from the chained route type. |
...
- **Hayate — First-party:** hayate-openapi 0.8.1 provides explicit source markers, portable typed constraints, typed binary files, response contracts, pluggable schema providers, OpenAPI 3.1.1, Scalar, and a generated dependency-free Fetch client on CPython and Python Workers. ([evidence 1](https://github.com/hayatepy/hayate/blob/main/benchmarks/ecosystem/runner.py), [evidence 2](https://github.com/hayatepy/hayate/blob/main/docs/ecosystem-compatibility.md), [source 3](https://github.com/hayatepy/hayate-openapi/blob/v0.8.1/README.md))
...
- **Hayate — First-party:** hayate-mcp 0.12.0 tests MCP 2026-07-28 on ASGI and native Python Workers with the official conformance runner. ([evidence 1](https://github.com/hayatepy/hayate/blob/main/benchmarks/ecosystem/runner.py), [evidence 2](https://github.com/hayatepy/hayate/blob/main/docs/ecosystem-compatibility.md))
...
- **Hayate — First-party:** hayate-openapi 0.8.1 generates a deterministic status-discriminated Fetch client with zero runtime package dependencies; create-hayate 0.13.0 drift-checks it, and golden-app strictly compiles and executes authenticated JSON, query/path, multipart, and delete calls against real ASGI. This is schema generation rather than Hono's in-process route-type inference. ([evidence 1](https://github.com/hayatepy/hayate/blob/main/benchmarks/ecosystem/runner.py), [evidence 2](https://github.com/hayatepy/hayate/blob/main/docs/ecosystem-compatibility.md), [source 3](https://github.com/hayatepy/hayate-openapi/blob/v0.8.1/README.md), [source 4](https://github.com/hayatepy/create-hayate/blob/v0.13.0/README.md), [source 5](https://github.com/hayatepy/golden-app/blob/842fd2486f3d8bd5c284e44e4f8528b0441a574a/client/check-api-client.ts))
